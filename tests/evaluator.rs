...
fn eval_goto_invalid_target_line_number() {
    ...
    assert_eq!(eval_result,
               Some("At 10, 8 invalid target line for GOTO".to_string()));
}
...
fn eval_goto_invalid_line_number() {
    ...
    assert_eq!(eval_result,
               Some("At 10, 8 GOTO must be followed by valid line number".to_string()));
}
...
fn eval_goto_no_line_number() {
    ...
    assert_eq!(eval_result,
               Some("At 10, 7 GOTO must be followed by a line number".to_string()));
}
...
fn eval_input_no_variable() {
    ...
    assert_eq!(eval_result,
               Some("At 10, 8 INPUT must be followed by a variable name".to_string()));
}
